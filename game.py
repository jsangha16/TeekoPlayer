...
class TeekoPlayer:
    ...
    def make_move(self, state):
        ...
        # TODO: implement a minimax algorithm to play better
            ...
            for s in success:
                ...
                row=s[0]
                col=s[1]
                c[row][col] = self.my_piece
                val= self.min_value(c, 0)
                alpha = -np.inf
                if (alpha <= val):
                    alpha = val
                    t = [(row, col)]
                            ...
                            for j in range(-1, 2):  
                                if 0 <= row + i < 5 and 0 <= col + j < 5: 
                                    if state[row + i][col + j] == ' ':  
                                        ...
                                        s[row + i][col + j] = self.my_piece
                                        s[row][col] = ' '
                                        t = [(row+i, col+j), (row, col)]
                                        move=t
    ...
    def max_value(self, state, depth):
        ...
        success = self.succ(state)
        for s in success:
            call = self.min_value(s[0], depth - 1)
            if call > alpha:
                alpha = call
            if alpha >= beta:
                break
    ...
    def min_value(self, state, depth):
        ...
        success = self.succ(state)
        for s in success:
            call = self.max_value(s[0], depth - 1)
            if call < beta:
                beta = call
            if alpha >= beta:
                break
